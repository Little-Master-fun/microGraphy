...
* 
* 主要功能：
* 1. 初始化SCH16TK10 IMU传感器，配置滤波器、灵敏度和抽取参数
* 2. 初始化IPS114屏幕显示模块
* 3. 循环读取IMU传感器数据（加速度、角速度、温度）
* 4. 将传感器数据实时显示在IPS114屏幕上
* 5. 提供传感器功能验证和数据监控功能
* 
* 使用方式：
* 在主函数中调用test_imu()函数即可开始测试
* 
* 硬件连接：
* - SCH16TK10传感器需要按照driver_sch16tk10.h中的引脚定义连接
* - IPS114屏幕需要按照zf_device_ips114.h中的引脚定义连接
* 
* 显示内容：
* - AccX/Y/Z: 三轴加速度值 (单位：m/s?)
* - GyrX/Y/Z: 三轴角速度值 (单位：dps)
* - Temp: 温度值 (单位：℃)
********************************************************************************************************************/
